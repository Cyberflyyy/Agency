...
const AboutUs = () => {
  return (
    <>
      <LeftSection />
      <div className="border border-white/15 min-h-[500px] flex flex-col items-center justify-center p-4 sm:p-6 xl:p-8">

      <div className="w-full flex flex-col items-center justify-center mb-6 sm:mb-8 xl:mb-12">
        {/* mały, kolorowy nagłówek */}
        <span className="text-sm sm:text-base xl:text-lg font-medium text-purple-500 uppercase tracking-wide mb-2">
          GOTOWY ?
        </span>
        {/* główny tytuł w dwóch wierszach, duża czcionka */}
          ...
          <div className="col-span-2">
            <div className=" bg-white backdrop-blur-sm border border-white/10 hover:border-white/30 transition-all duration-300 p-3 sm:p-4 xl:p-6 w-full flex flex-col rounded-bl-2xl rounded-br-2xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-0.5 flex-1 rounded-2xl overflow-hidden ">
                {[
                  { icon: <Rocket className="h-8 w-8 text-purple-400" />, name: "Next.js", desc: "Framework React dla nowoczesnych aplikacji" },
                  { icon: <Layout className="h-8 w-8 text-cyan-400" />, name: "Tailwind CSS", desc: "Utility-first CSS dla szybkiego projektowania" },
                  { icon: <Database className="h-8 w-8 text-emerald-400" />, name: "Prisma", desc: "Zaawansowana baza danych relacyjna" },
                  { icon: <Server className="h-8 w-8 text-amber-400" />, name: "Node.js", desc: "Środowisko uruchomieniowe JavaScript" },
                  { icon: <Code className="h-8 w-8 text-rose-400" />, name: "TypeScript", desc: "Typowany JavaScript dla lepszej jakości kodu" },
                  { icon: <Zap className="h-8 w-8 text-blue-400" />, name: "Vercel", desc: "Platforma do wdrażania aplikacji webowych" },
                ].map((tech, index) => (
                  <div
                    key={index}
                  ...
                  >
                    {tech.icon}
                    <h4 className="font-medium text-white text-sm sm:text-base">{tech.name}</h4>
                    <p className="text-white/70 text-xs sm:text-sm">{tech.desc}</p>
                  </div>
                ))}
              </div>
            </div>
